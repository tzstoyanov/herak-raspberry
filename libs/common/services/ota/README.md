...
[fota](https://github.com/JZimnol/pico_fota_bootloader) bootloader is used to boot the device. The `pico_fota_bootloader.uf2` file is generated during the compilation, and on first install must be copied on the device using the `BOOTSEL` button method. When `pico_fota_bootloader` boots, the application image `<device name>.uf2` can be copied using the same `BOOTSEL` button method. A few additional files are generated during the compilation, that can be used for OTA updates:  
...
- `<device name>.meta` - Metadata of the image - target platform, sw version, hash, build date, image name and file name. This file is used from OTA logic to check if the new image should be used for automatic upgrade.
...
- `check:tftp://<server>[:<port>]/[<file>]` - Check for updates on tftp `<server>:[port]`, looking for meta `[<file>]`. If `<file>` is not specified, a file with name `<application image>.meta` is used. If new image is found, it is not applied automatically. The metadata file must exist on the TFTP server.  
