...
/* Defining 'cron_' time functions to use UTC (default) or local time */
 ...
 */
static unsigned int find_next(uint8_t *bits, unsigned int max, unsigned int value,
							  struct tm *calendar, unsigned int field, unsigned int nextField,
							  int *lower_orders, int *res_out)
{
	int notfound = 0;
	int err = 0;
...
static unsigned int find_next_day(struct tm *calendar, uint8_t *days_of_month,
								  unsigned int day_of_month, uint8_t *days_of_week,
								  unsigned int day_of_week, int *resets, int *res_out)
{
	int err;
...
static void set_number_hits(const char *value, uint8_t *target, unsigned int min,
							unsigned int max, const char **error)
{
	unsigned int i1;
	size_t len = 0;
	size_t i;
...
time_t cron_next(cron_expr *expr, time_t date)
{
	...
	if (original == CRON_INVALID_INSTANT)
		return CRON_INVALID_INSTANT;
	...
	if (calculated == CRON_INVALID_INSTANT)
		return CRON_INVALID_INSTANT;
 ...
 * and reset the calendar.
 */
static unsigned int find_prev(uint8_t *bits, unsigned int max, unsigned int value,
							  struct tm *calendar, unsigned int field, unsigned int nextField,
							  int *lower_orders, int *res_out)
{
	int notfound = 0;
...
static unsigned int find_prev_day(struct tm *calendar, uint8_t *days_of_month,
								  unsigned int day_of_month, uint8_t *days_of_week,
								  unsigned int day_of_week, int *resets, int *res_out)
{
	unsigned int count = 0;
