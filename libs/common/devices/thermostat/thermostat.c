...
static int therm_mqtt_dvice_send(struct thermostat_context_t *ctx, int idx)
{
	...
	ADD_MQTT_MSG_VAR(",\"hysteresis\": \"%3.2f\"", dev->off_t - dev->on_t);
	ADD_MQTT_MSG("}")
...
static void therm_mqtt_init(struct thermostat_context_t *ctx)
{
	...
	for (i = 0; i < ctx->dev_count; i++) {
		...
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].module = THERMOSTAT_MODULE;
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].platform = "sensor";
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].dev_class = "temperature";
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].unit = "°C";
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].value_template = "{{ value_json['hysteresis'] }}";
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].state_topic =
					ctx->devices[i]->mqtt_comp[THERM_MQTT_STATE].state_topic;
		sys_asprintf(&ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].name, "hysteresis_%d", i);
		mqtt_msg_component_register(&(ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS]));
		ctx->devices[i]->mqtt_comp[THERM_MQTT_HYSTERESIS].force = false;
	}
}
								  ...
								  int idx, float temperature, bool hist)
{
	...
	if (hist) {
		...
		f = ctx->devices[idx]->off_t - temperature;
		if (f != ctx->devices[idx]->on_t)
			update = true;
		ctx->devices[idx]->on_t = f;
		if (IS_DEBUG(ctx))
		...
		f = temperature - f;
		if (temperature != ctx->devices[idx]->off_t || f != ctx->devices[idx]->on_t)
			update = true;
...
static int cmd_therm_state(cmd_run_context_t *ctx, char *cmd, char *params, void *user_data)
{
	...
	state = (bool)strtol(rest, NULL, 0);
	if (id >= 0 || id == -1)
		ret = therm_set_state(wctx, id, state);
...
static app_command_t therm_requests[] = {
	{"state", ":<thermostat_id>:<0/1> - disable / enable thermostat", cmd_therm_state},
